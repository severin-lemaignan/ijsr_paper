...
The authors would like to thank the reviewers for their constructive feedback that helped us to strengthen the paper.

Below follow our answers to the reviewersÕ comments and suggestions. Each answer also indicates the changes made in the paper to address the reviewerÕs concern. An important number of language corrections are made, and the paper is proof-read by a native English speaker.
...
A mention to the Dora robot has been added in section 2.1.

\subsection{``Give examples of the alignment of ORO with OpenCyc''}

Numerous predicates in {\tt commonsense.oro.owl} are defined in the 
OpenCyc namespace: {\tt farFrom, near, hold-Underspecified, objectFoundInLocation}.

Similarly, most of the upper classes in the ontology belong to OpenCyc 
(like {\tt SpatialThing, PartiallyTangible, TemporalThing}...).

Overall, about 50 entites (on the >180 defined in {\tt commonsense.oro.owl}) 
come from OpenCyc.
...
Kelleher's work, as well as Blisard's, are now correctly cited in the section.
...
The detailled algorithm is provided in appendix to this letter.
We would be happy to add it to the paper as well if the editor want to consider it.
