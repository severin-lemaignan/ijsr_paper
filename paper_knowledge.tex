...
We discuss in section~\ref{modeling_real_world} how the initial knowledge (the
existence of a yellow banana) can be acquired.
...
while indicating: ``That box'' (Figure~\ref{fig|pointing}b). The robot's
...
Note also that, as mentioned in section~\ref{spygame}, perception issues have
been skipped in our experiments by using a tag-based object identification
...
ontologies (with facts like \stmt{BANANA1 type Banana} where \concept{BANANA1}
...
It would be interesting to broaden this approach with automatic classifiers
...
that this very object is in fact a banana. A similar approach has been 
