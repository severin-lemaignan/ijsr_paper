...
considered widely accepted among the inter-actors (\eg ``bottles may contain
...
refers to the fact that several, \textit{a priori} non correlated, facts
must be associated to the same common concept. This process requires the
...
Besides, the following active processes would be desirable to:
\begin{itemize}
...
statements are continuously reasoned about (\emph{classified}), and at anytime,
...
Available predicates allow to describe the state of the agents and the world
...
Daoutis and Tenorth rely on the same \textsc{Cyc} concepts\fixme{Give
examples}).
...
\begin{itemize}
	...
	\item standard SPARQL\footnote{{\em SPARQL Protocol and RDF Query Language}, \url{http://www.w3.org/TR/rdf-sparql-query/}} queries.
\end{itemize}
...
helps building assertions that are related to previous one by offering
...
new, separate, in-memory OWL model for that agent. Thus, different robot's
...
allows to store and reason on different (and possibly globally inconsistent)
models of the world.
...
the belief state of its inter-actors, opening new possibilities for the control
architecture. In the section~\ref{dialog} we present how we use this feature to
make sense of the user sentences from the his/her point of view. Moreover,
these multiple models can be viewed as well as different interpretive scopes,
allowing to interpret the same reality from different points of view.
...
``bottle'' whereas two bottles are currently visible. Discrimination routines
