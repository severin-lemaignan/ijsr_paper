...
``Jido, give me this'', says Thomas while looking at a box that contains a
...
interaction: what are the prerequisites for such a human sentence --``Jido,
give me this''-- to be understood by the robot, correctly interpreted in the
...
acts in Searle~\cite{Searle1976} classification.
...
architecture for indoors service robots. 
...
a \emph{grounded situation model} which is an a-modal model of the world where
...
used in combination with the \textsc{CycL} language. This enables second-order
...
of various advanced cognitive behaviours like learning new concepts or
interactive objects discrimination.
...
between perceived objects in real-time and it allows to virtually \emph{view}
the same scene from different points of view, enabling \emph{visual} and
...
(including both humanoid robot and service robots), demonstrating the
...
Section 5 presents three use-cases that were conducted on three different
