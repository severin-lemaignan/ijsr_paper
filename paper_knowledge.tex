...
Anchoring perceptions in a symbolic model requires first perception abilities
and second, a symbolic assessment of the percepts. In this section we present
SPARK (\emph{SPAtial Reasoning \& Knowledge}), a situation assessment reasoner
that generates relevant symbolic information from the geometry of the
environment with respect to relations between objects and human capabilities.
Moreover, the notion of \emph{Perspective
Taking}~\cite{Flavell1992,Tversky1999} is employed in the heart of the
reasoning to provide the robot with the ability to put itself at the human's
...
point, etc. In our context, we group robots and humans into a single category.
...
\subsection{Locations} As humans, one way of referring to object's
...
existence is hinted by verbal assertions from a human), SPARK offers a richer
3D model that enables the computation of several spatial relationships between
