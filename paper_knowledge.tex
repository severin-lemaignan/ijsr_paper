...
\institute{ 
	Séverin Lemaignan \and Raquel Ros \and E. Akin Sisbot \and Rachid Alami
	\at
	CNRS - LAAS, 7 avenue du Colonel Roche, F-31077 Toulouse, France\\
	Université de Toulouse, UPS, INSA, INP, ISAE, LAAS, F-31077 Toulouse, France\\
	\email{\{slemaign, rrosespi, sisbot, rachid\}@laas.fr}
	\and
	Séverin Lemaignan \and Michael Beetz
	\at
...
is suitable for later high-level tasks like dialogue understanding. We show how
...
knowledge representation approach based on Description Logics, and experiments
...
\begin{figure}%[!ht] 
	\centering
	\includegraphics[width=0.9\linewidth]{images/pt.jpg} 
...
Ultimately, an implicit speaker's expectation can sometimes be conveyed --- the
\emph{perlocutionary act} --- that must be taken into account. If Thomas would
have said instead `` -- Jido, this box is too heavy for me'', he implicitly
expects the robot to memorize the fact that the box's weight is over his
...
and maintaining a coherent model of the physical world. This model is realistic
in the sense that it relies on accurate 3D models of both manipulated objects
and humans. It also has dedicated mechanisms to manage disappearing or occluded
...
property \concept{isVisible \textbf{true}} and \concept{isVisible \textbf{false}}, in
...
OWL ontologies (a decidable subset of the predicate logics).

Used in combination with the situation assessment framework, the robot is thus
able to maintain different symbolic models of the world, one per agent. This proves an
...
for situated verbal interaction, integrated into a broader control architecture
for indoors service robots. 
...
Sloman~\cite{Sloman2007} who criticizes the \emph{``concept emprism"} that
...
research field. In~\cite{Roy2005}, Roy and Reiter summarize what they see as the main
...
and the ability to take into account the human perspective. This list offers an
interesting entry point to evaluate our contribution.
...
bi-directionality of the interaction and context building. They point out as well
...
Brick and Scheutz~\cite{Brick2007} have contributions regarding natural language
...
by Shapiro and Bona~\cite{Shapiro2009} is an architecture
explicitly built to tackle the grounding issue from the percept to the
decision. It is a three-layers architecture: a \emph{Knowledge Layer}, a
low-level \emph{Sensori-Actuator Layer} and an intermediate
\emph{Perceptuo-Motor Layer} that binds the previous two.  The knowledge layer
relies on a custom knowledge representation language (more expressive than
first-order logic), and natural language processing capabilities similar to
ours are available. The GLAIR project has been only demonstrated in a limited
set of environments, but exhibits interesting features like explicit management
...
framework for robotics. They try to tackle the grounding issue by storing low-level facts
(like SIFT visual features) in a layered symbolic architecture that works well
in simple sensori-motor spaces. However this approach raises concerns regarding
scalability and management of more complex entities or interactions.

Daoutis et al.~\cite{Daoutis2009} introduce one of the first complete
architecture for grounded human-robot interaction. They successfully bind
...
a dedicated anchoring module. They base their knowledge
model directly on the \textit{ResearchCyc} ontology (including the
\textit{MicroTheories} concept), used in combination with the \textsc{CycL}
language. This enables second-order logic modeling and access to a large
common-sense knowledge base.

Beetz et al.~\cite{Beetz2010} proposes a cognitive architecture called \textsc{CRAM}
(Cognitive Robot Abstract Machine) that integrates
...
used in our work is limited to binary predicates), it is based on the closed
world assumption (if something cannot inferred to be true, it is inferred to be
false) whereas we rely on the open world assumption, which is more realistic in
real world scenarii. A probabilistic extension of \textsc{KnowRob}, called
\textsc{ProbCog}~\cite{Jain2009} is available. While in principle possible, the
CRAM architecture does not provide currently explicit support for the
...
symbolic knowledge repositories, our work extends these previous
contributions by tackling more realistic human-robot interactions: open
speech; complex, dynamic, partially unknown human environments; fully
embodied (with arms, head, ability to move,...) autonomous robots that can
perform manipulation.

Four specific contributions are presented in this work: first, we introduce in
this paper a novel and versatile knowledge base that models in a
formal framework, based on first-order logics, not only the robot's own beliefs
but also every other cognitive agent the robot interacts with.  This explicit
modeling of other agents' belief states is used for the interaction and eases
the implementation of various advanced cognitive behaviours like learning
new concepts or interactive objects discrimination.
...
on-line by merging sensor modalities. It computes in real-time spatial
relations between perceived objects and it allows to virtually \emph{view} the
same scene from different point of view, enabling \emph{visual} and
...
on situated language grounding. We present several examples that include a
...
layered architecture because components interactions are mostly bidirectional.
This is especially visible for the dialogue input processing: this component
does not simply act as an alternative perceptual input to the symbolic
database, but also actively queries previously acquired knowledge to
...
These points highlight some original aspects of a larger cognitive
architecture that has been deployed and tested on three distinct robotic
platforms (including both humanoid robot and service robots), demonstrating the
...
Section 3 presents the {\sc SPARK} (for \emph{SPAtial Reasoning \& Knowledge})
module that merges perceptual information with a coherent geometric model and
builds a symbolic interpretation of the world from the robot point of view. It
also computes an individual symbolic model for each agent currently present in
the environment. 

Section 4 covers in details the dialogue processing component. The main
algorithms for natural language grounding are detailed. Interactions between
this component and the knowledge base are described.

Section 5 presents three experiments that were conducted on three different
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{ORO, a knowledge management platform}
\label{cognitivekernel}

\subsection{On knowledge representation}
\label{knowledge_representation}

While \emph{knowledge} has no general definition that researchers agree on, for
our own purposes we define knowledge as \emph{an information interpreted in the
cultural and social context of the robot}, where an information is a
\emph{statement} or an \emph{assertion} on the world\footnote{In this paper,
statements are always triples \texttt{<subject, predicate, object>}, following
the Description Logics formalism.}. In practical terms, knowledge is made
of statements that are \emph{contextualized}, if possible \emph{synthesized}
and \emph{limited} to a domain of validity.

These three features have important consequences on the way a knowledge
representation and storage system must be designed. Let us examine them:

\paragraph{Contextualizing} The information contextualizing is the ability for
a cognitive system to connect a fact with a \emph{cultural context}, an
\emph{interpretive scope} and the set of other facts previously acquired by
the agent.
...
considered as widely accepted amongst the interactors (like \textit{bottles may
contain water}). This knowledge is often referred as \emph{common-sense
knowledge}.

By \emph{interpretive scope}, we mean that a concept may have different
interpretation depending on the agent, the current situation or the time frame
...
While several approaches to build contextualized knowledge are proposed in this
...
\paragraph{Synthesis} The synthesis of knowledge is the identification process
of facts (and their components, concepts and predicates) with each others. For
instance, if the robot observes a human sitting down at a table, and at the
same time, we tell it that ``Peter is sitting at the table'', we would like the
robot to infer that ``Peter'' may be the name of the human. \textit{Synthesis}
refers to the fact that several, \textit{a priori} non correlated, facts must
be associated to the same common concept. This process requires the ability to
control the logical consistency of the knowledge corpus. For instance, if we
add to the previous example the fact that the human that is sitting is a woman,
the synthesis ``Peter is the name of the human'' is not valid anymore.

\paragraph{Domain of validity} The domain of validity of an information,
lastly, covers several aspects: temporal, situational and probabilistic. The
main role of the domain of validity is to specify the scope in which an
information is (believed to be) true. While related to the previously concept
of \emph{interpretive scopes}, the domain of validity addresses the
question whether a fact must be or not considered in a given context. This
limitation of the validity is usually not carried by the fact itself. In the
previous example, for instance, the robot observes a human sitting at a table.
This fact ``a human is sitting at the table'' is true only for a limited
period, until the human stand up. This period of time is not directly
accessible (the robot does not know how long the human plans to stay), but the
knowledge representation system must be able to deal with this uncertainty and
...
%for instance. The difficuly lies in the selection of the relevant situation in
%which reasonning must be carried out at a given time) and have currently to be
%defined in the cultural background of the robot.

Those three aspects translate into a set of features that are desirable for a knowledge representation system:
\begin{itemize}
	\item represent raw information,
	\item render a general cultural background, in form of common-sense knowledge,
...
Besides, active processes would be desirable to:
\begin{itemize}
	...
	\item monitor contexts and manage accordingly the validity of the stored knowledge,
	...
	inherently inconsistent. In this article, we however make a
...
identify where knowledge is lacking and actively trigger behaviours to acquire
it---, to give some examples, could arguably be added to the list. However,
this first analysis sets a convenient reference frame to understand and
evaluate knowledge representation systems, including the \textsc{ORO} knowledge
...
\centering
  ...
  knowledge model of the Jido robot during an experiment. Dots represent
...
knowledge storage service implemented as a server: at run-time, the robot
components can add or query statements. Figure~\ref{fig|oro-overview}
...
(detailed thereafter). External plugins can also be easily hooked. The modules
rely on several parallel ontology \emph{back-ends} where the knowledge is
actually stored.

Knowledge is represented in ORO in the Description Logics formalism (using the OWL-DL -- \emph{Web Ontology Language - Description Logics} -- language), as RDF (\emph{Resource Description Framework})
triples (for instance \stmt{robot isIn kitchen}). We use the
Jena\footnote{\url{http://jena.sourceforge.net/}} framework as the underlying
library to load and build in-memory OWL model. We use it in conjunction with
the Pellet\footnote{\url{http://clarkparsia.com/pellet/}} reasoner to ensure
the continuous classification of the OWL concept graph: at run-time, newly
added statements are continuously reasoned about (\emph{classified}), and at
anytime, the server expose a complete set of statements, both the asserted ones
and the inferred ones. For instance, if \stmt{socrates type Human}
and \stmt{Human subClassOf Mortal} are asserted, the server
transparently adds the inferred statement \stmt{socrates type
Mortal}, and a later query retrieving all mortal entities would return
\concept{socrates} as well. The language of the OWL family use the \emph{Open World
Assumption} (if a fact can not be inferred as true, it does not mean that it is
inferred to be false), and the Pellet reasoner honors this assumption as well.
...
and logical properties that are later used for inference.

The \emph{OpenRobots Common Sense Ontology} defines a small set of classes (56 are
currently defined) and predicates (60 are currently defined) focused on
concepts useful for robot interaction with humans. It includes both very broad
categories like \concept{SpatialThing}, \concept{Event} or \concept{Action}, and much more
concrete concepts as \concept{Table}, \concept{Book} or colors. Predicates allow to
describe the state of the agents and the world with relations like \concept{isOn},
\concept{sees}, \concept{currentlyPerforms}, etc.
...
Our common sense ontology is closely aligned on the open-source
\textsc{OpenCyc}\footnote{\url{http://www.opencyc.org}} upper ontology.
\textsc{OpenCyc} defines a large taxonomy of concepts and semantic
...
of knowledge from these other sources. Relying on the \textit{de facto}
\textsc{OpenCyc} standard enables as well knowledge exchange with other robots
(for instance, the works previously mentioned by Daoutis and Tenorth rely on
the same \textsc{Cyc} concepts).
...
restrictions (however, not the SWRL rules) at run-time. This allows
...
features to manage knowledge at higher level: independent cognitive models for
...
\label{base}
ORO offers an extended set of methods for processing of facts at the triples level. It includes:
\begin{itemize}
	\item statement ({\it i.e.}, RDF triples) insertion, removal, update,
	\item pattern-based removal of statements,
	...
	\item fast concept lookup, with multi-lingual support,
	\item queries with patterns (for instance, \stmt{ * isOn
	table}, which means ``return me all objects on table'') and filters (for
	instance, \concept{weight < 150.0}),
	\item standard SPARQL\footnote{\url{http://www.w3.org/TR/rdf-sparql-query/}} queries.
\end{itemize}

These basic functionalities enable the incremental construction (and
exploitation) of a consistent knowledge model, and the \emph{Common Sense Ontology}
core set of concepts helps to build assertions that are related to previous one by offering
...
new, separate, in-memory OWL model. Robot's components like supervision or
situation assessment may then store the agents' beliefs in a
separate model. All knowledge processing functions available on the robot
primary model are equally available in every agent model. This allows to store
and reason on different (and possibly globally inconsistent) models of the
world.
...
architecture. We present in the section~\ref{dialog} on natural language
grounding how we use this feature to make sense of the human sentences from the
human own point of view.

These multiple models can be viewed as well as different interpretive
scopes, allowing to interpret the same reality from different points of
view.
...
when interacting with a user, the robot often needs to clarify an ambiguity
produced by its human partner. For instance, a user may refer to a ``bottle''
whereas two bottles are currently visible: the discrimination routines can
identify possible (symbolic) differences (maybe the color or the size of the
bottles) that permit the robot to ask an accurate question to the user to solve
the ambiguity. This discrimination can occur from the robot perspective or from
a specific agent perspective. Usage of these categorization abilities are
illustrated in the \emph{Spy Game} experiment (section~\ref{spygame}).
...
\subsubsection{The events framework} \label{events} Lastly, ORO allows external
modules to be triggered when specific events occur. For instance, when a
logical sentence becomes true or false, or if a new instance of a certain class
is added. One immediate application is reactive supervision: a component could
for instance subscribe to events of kind \setstmt{?agent isVisible true, ?agent type Human}. As soon as the perception layer detects a human in the
robot's field of view and accordingly updates the knowledge base, the
supervision component would be triggered back.

The event framework also takes advantage of the inference capabilities of ORO.
Thus an event can be indirectly triggered if its triggering conditions can be
inferred to be true.

Next sections present how symbolic knowledge is actually produced and added to
ORO.
...
their symbolic assessments. In this section we present SPARK (\emph{SPAtial
Reasoning \& Knowledge}), a situation assessment reasoner that generates
relevant symbolic information from the geometry of the environment with respect
to relations between objects, robots and humans. Moreover, the notion of
\emph{Perspective Taking}~\cite{Flavell1992,Tversky1999} is employed in the
heart of the reasoner to provide the robot with the ability to put itself at
the human’s place and to reason about the world from different perspectives.

As mentioned in the introduction, this paper do not focus on the sensor-level
...
their capabilities in a given situation: they can both reach, grasp, look,
point, etc. In our context, we group the robot and the human into a single
category. Thus, we define the ``agent'' as an entity that can act in the
environment and manipulate it. In this work we focus on the following
capabilities from each agents' perspective \footnote{Note that each of the
capabilities described are computed from each agent point of view, and
therefore, also stored in different models in ORO for further use at the
decisional level.}.
...
on, \ie, where its focus of attention is directed to. This model is based on a
...
this example only the grey box satisfies the \verb"looksAt" relation.
...
\item \emph{Points At}: verifies if an object is pointed at by an agent. This
...
outcome of the evaluation will result only in one relation, \ie, \stmt{Agent
\textbf{pointsAt} BigObject} since the small one is not visible to the agent.
On the contrary, if the small object is in front of the big one, then both
objects will be pointed at by the human, generating an ambiguity which should
be solved through higher level reasoning (\eg context analysis or
clarification through verbal interaction as shown later on).

\item \emph{Reachable}: it allows the robot to estimate the agents capability
...
While the first three relations (\emph{sees, looksAt} and \emph{pointsAt}) are
computed through a model based approach, the latter one is based on the
Generalized Inverse Kinematics with pseudo inverse
...
\subsection{Locations} As humans, one way of referring to the object's
positions is based on their symbolic descriptors instead of using their precise
position. In fact, in many cases, this information is the most precise one that
we can give since we do not store the numeric coordinates of objects. The
following relations are computed with respect to the position of the agents and
the objects.

\begin{itemize} \item \emph{Location according to an agent} The predicate
\verb"isLocatedAt" represents spatial locations between agents and objects. For
example we say ``it is on my right, on your left,...'' We compute these spatial
...
\emph{far}, may also be considered. The number of regions and proximity values
can be chosen depending on the context where the interaction takes place.

\item \emph{Location according to an object} We can also refer to object
locations with respect to other objects in the environment, such as, above,
next to, in, etc. These types of relations are widely studied in language
grounding (e.g.~\cite{Regier01} presented different models to define the
...
(Figure~\ref{fig::sprelation}): \begin{itemize}

\item \textit{isOn}: computes if an object $O_1$ is on another object $O_2$ by
evaluating the center of mass of $O_1$ according to the bounding box of $O_2$.

\item \textit{isIn}: evaluates if an object $O_1$ is inside another object
$O_2$ based on their bounding boxes $BB_{O_1}$ and $BB_{O_2}$.

\item \textit{isNextTo}: indicates whether an object $O_1$ is next to another
...
objects (e.g. two houses) to consider them as being next to each other is much
...
SPARK can be compared to the \emph{Grounded Situation Model} (GSM) by
Mavridis and Roy~\cite{Mavridis2005} in the sense that they both provide an amodal physical
representation of the world used as mediator between the sensor space and
symbolic models. They have however a different set of features: while GSM
enable representation of time and imaginary objects (whose existence is hinted
by verbal assertions from a human), SPARK offers a richer 3D model that enable
computation of several spatial relationships between objects and an effective
implementation of the perspective taking.
...
(we call it \emph{resolution}) of concepts involved in the human discourse
through the robot's own knowledge.
...
moderately complex sentence (``remember that I want you to give me my nice blue
bottle.'').
...
verbal groups are resolved as well, and their associated
\emph{thematic roles} are retrieved (verbal phrase resolution).
...
Since the robot tries to ground a human utterance, all queries are sent to the
\emph{human} model, \ie the sentence is interpreted from the human perspective. 
...
\begin{figure}
	...
	\hspace{0.3cm}\stmt{?obj type Banana} \\
    	...
    	\hspace{0.7cm}$\Rightarrow$ {\tt ?obj = banana\_01}\\
...
We want first to match the nominal group \emph{The yellow banana} to an already
...
algorithm of nominal groups is described by the algorithm~\ref{algo|Resolution}.
...
(algorithm~\ref{algo|Resolution}, \emph{(\ref{res.desc})}). In the example, a banana
(\stmt{?obj type Banana}) that is yellow (\stmt{?obj hasColor yellow})\footnote{Predicates like \concept{hasColor} or \concept{hasSize} that
...
\emph{Thematic roles} allow to semantically link a verb to its complements.  
There is no general agreement amongst linguists on a comprehensive list of 
thematic roles. The amount and the granularity of the roles varies a lot in 
the literature~\cite{Gutierrez2001}. We use a small set of them that matches
...
of actions. For each action we identify and store the role of the subject of
...
\concept{myself} (\concept{myself} always represent the robot itself). When
present, anaphora (references to previous concepts like ``give me the banana, I
...
augmented, sentence (``Give me the green banana") goes again through all 
...
\parbox[c]{4.3cm}{\includegraphics[width=4.3cm]{images/kimp1.jpg}} \hspace{0.2em}
\parbox[c]{4.1cm}{\includegraphics[width=4.1cm]{images/rviz.png}}
\flushleft
\hspace{1.8cm} (a) \hspace{3.9cm} (b)
\caption{(a) Rosie, looking for objects it may know, and (b) viewed in Willow Garage's RViz. The clusters of point are given a unique identifier by the perception that allows the supervision to create the link between the physical objects and their symbolic representation in ORO.}
...
The robot picks an unknown object from the table, shows it to the user, and
asks about its name and type (Figure~\ref{fig|kimpwatching}). The user
interacts with the robot through a dedicated bridge, allowing to chat with
...
type it already knows: by asking in loop the human for the categories of an
object until it can connect it to a concept it already knows, the robot
...
\centering
	...
	\chatN{human} & "Rosie's mug" \emph{[Rosie adds \stmt{OBJ001 label "Rosie's mug"} stmt]}\\
	\chatN{rosie} & What kind of object is Rosie's mug? \\
	\chatN{human} & Mug \\
	\chatN{rosie} & What is a Mug? \emph{[Rosie adds \stmt{OBJ001 type Mug} but it does not know the \emph{Mug} concept]}\\
	\chatN{human} & Tableware \emph{[Rosie knows about \emph{Tableware} - belongs to the common-sense ontology. It adds \stmt{Mug subClassOf Tableware}]}\\
	 & (...) \\
...
\subsection{``Spy Game'' experiment}
...
\parbox[c]{4.3cm}{\includegraphics[width=4.3cm]{images/spy-game-real.jpg}} \hspace{0.2em}
\parbox[c]{4.1cm}{\includegraphics[width=4.1cm]{images/spy-game-mhp.jpg}}
\flushleft
\hspace{1.8cm} (a) \hspace{3.9cm} (b)
...
(\concept{front}, \concept{back}, \concept{left}, etc, and \concept{in}, \concept{on} and \concept{next
to}) and visibility (only visible objects for both agents can be considered in
the game) are automatically computed on-line by \textsc{SPARK} (the geometric
reasoner). Figure~\ref{fig|spyGameExample} shows an example of a round game.
...
\centering
	\begin{tabular}{r|p{6cm}}
		...
		\chatN{jido} & \emph{[retrieves possible objects: coke-bottle, yellow-bottle, orange-bottle, cup-with-handle]} \\
	 			& \emph{[keeps visible objects: coke-bottle, yellow-bottle, cup-with-handle]}\\
				& \emph{[obtains discriminants: type, color.]}\\
				& Which type of object is: bottle or cup? \\
		\chatN{human} & Bottle.\\
		\chatN{jido} & \emph{[obtains possible objects: coke-bottle, yellow-bottle.]}\\
				& \emph{[obtains discriminants: color.]}\\
				& What color the object is: red or yellow?\\
		\chatN{human} & Red.\\
...
In order to illustrate some of different reasoning abilities of the robot,
based on the approach presented in this paper, we have designed the following
...
keyboard). Ideally, the robot should also perform actions when required (e.g.
...
to the Spy Game scenario, perception of objects is done through a tag-based
...
First Tom enters the room and places on the table some of the things to pack: a
...
The initial information in the ontology corresponds to:
...
\begin{center}
    \begin{tabular}{l}
      ...
      \stmt{videoTape1 type VideoiTape}\\
...
inside the box next to Jido and the leaves. (Figure~\ref{fig|sitAssessRoman}b):
...
\begin{center}
\hspace{0.7cm}$\Rightarrow$ {\tt ?obj = [videoTape1, videoTape2]}
\end{center}
...
ontology ({\tt pointsAt(?ag, ?obj) $\land$ looksAt(?ag, ?obj) $\to$
...
\hspace{0.7cm}$\Rightarrow$ {\tt ?obj = videoTape2}\\
\end{tabular}
\end{center}
...
\hspace{0.7cm}$\Rightarrow$ {\tt ?obj = [videoTape1]}
\end{tabular}
\end{center}
...
deaf robot has a good vision system but can not hear anything.  Without any
changes to the software architecture above the language processing module or
the vision module, supervisors of both robot would be able to perform equally
well.
...
especially well suited to set up \emph{False-Belief}
experiments~\cite{Leslie2000}, and we plan to further explore those kinds of
applications.
...
Wikipedia) to an unambiguous, context-dependent, internal concept. This very
concept can (or not) be \textit{a priori} available to the robot as a
...
with a human: because the robot and the human share the same physical
...
a broader symbolic system: our architecture is not bound to the morphology or
...
sees the human, and is able to deduce, at least partially, what the human
perceives or not. This relies on the embodied nature of the interaction. We
infer that a human focuses on some object because it points at it, looks at it,
and besides, the object is visible to him. This relies as well on the embodied
nature of the interaction. And this allow us, in turn, to understand the
...
Let conclude this article by a short glance back at Roy's list of challenges
for human-robot dialogue: 
\begin{itemize}

	\item While more modalities (especially, deictic gestures and social gazes)
	can be added, we have proposed in this article a \emph{cross-modal
	representation system},

	\item One of the main feature of the \textsc{Dialogs} module is its ability
	to interactively ground concepts throught disambiguation, bringing the
	...
	trying to develop even futher towards psychology-inspired experiments.
